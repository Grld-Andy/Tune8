...
const Songs: React.FC = () => {
  ...
  const allSongs = useRef<SortedSongs>({})
  useEffect(() => {
    allSongs.current = getSortedSongs(songs, sortOrder)
  }, [sortOrder, songs])
        ...
        <div className="songs view">
          {
            ...
            sortOrder={sortOrder}
            object={allSongs.current} closeAndScroll={closeAndScroll}/>
          }
          {
            Object.keys(allSongs.current).sort().map(letter => (
              <section key={letter} id={letter}>
                <h2 onClick={toggleShowNav}>{letter}</h2>
                  <div className="listItem">
                    {
                      Array.from(allSongs.current[letter]).sort((a, b) => a.tag.tags.album.localeCompare(b.tag.tags.album)).map((song, index) => (
